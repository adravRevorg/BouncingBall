...
float collisionDetection(){
  ...
  float d0 = n.dot(PVector.sub(x0,plane)) + r;
  float d1 = n.dot(PVector.sub(x,plane)) - r;
...
boolean atRest(){
  ...
  float d = n.dot(PVector.sub(x,plane)) + r;
  if (abs(d)>100)    return false;              //not near enough to the plane to consider rest  (Condition 2 in book)
  
  if (PVector.dot(a,n)>0.0001)  return false;                           //force isnt opposite to plane, ex : vertical plane   (Condition 3 in book)
  
  PVector aN = PVector.mult(n,n.dot(a));
